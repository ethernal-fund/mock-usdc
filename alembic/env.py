...
def _get_db_url() -> str:
    """Obtiene la URL de DB desde variable de entorno o alembic.ini."""
    url = os.environ.get("DATABASE_URL") or config.get_main_option("sqlalchemy.url")
    if not url:
        raise ValueError("DATABASE_URL no configurada")

    # Normalizar a asyncpg
...
def _get_ssl_context() -> ssl.SSLContext:
    """SSL permisivo para Supabase + Render free tier (IPv4 only)."""
...
def run_migrations_offline() -> None:
    ...
    Útil para revisar los scripts antes de aplicar.
    """
    url = _get_db_url()
    context.configure(
        url=url,
